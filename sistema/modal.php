...
if (!empty($_POST)) {
  ...
  if ($_POST['action'] == 'infoProducto') {
    ...
    $query = mysqli_query($conexion, "SELECT id_producto, descripcion, precio, existencia FROM productos WHERE id_producto = $producto_id");
...
// Eliminar Producto
  if ($_POST['action'] == 'delProduct') {
    ...
    $query_delete = mysqli_query($conexion, "UPDATE productos SET estado = 0 WHERE id_productos = $idproducto");
...
if ($_POST['action'] == 'searchForDetalle') {
  ...
  if (empty($_POST['user'])){
    ...
    $query = mysqli_query($conexion, "SELECT tmp.correlativo, tmp.token_user,
      tmp.cantidad, tmp.precio_venta, p.id_producto, p.descripcion
      FROM detalle_temp tmp INNER JOIN productos p ON tmp.codproducto = p.id_producto
    ...
    while ($data = mysqli_fetch_assoc($query)) {
      ...
      $sub_total = round($sub_total + $precioTotal, 2);
      $total = round($total + $precioTotal, 2);
// <td>'.$data['codproducto'].'</td>  linea 175
        $detalleTabla .= '<tr>
...
if ($_POST['action'] == 'delProductoDetalle') {
  if (empty($_POST['id_detalle'])){
    ...
    while ($data = mysqli_fetch_assoc($query_detalle_tmp)) {
      ...
      $sub_total = round($sub_total + $precioTotal, 2);
      $total = round($total + $precioTotal, 2);
// <td>'.$data['codproducto'].'</td> linea 249
        $detalleTabla .= '<tr>
            <td>'.$data['id_producto'].'</td>
            <td colspan="2">'.$data['descripcion'].'</td>
            <td class="textcenter">'.$data['cantidad'].'</td>
